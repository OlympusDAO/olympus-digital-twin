def p_reserves_in(params, substep, state_history, state) -> dict:
    ...
    if day % 7 == 0:  # Rebalance once a week
        ...
        if state["target_liq_ratio_reached"] is False:
            max_outflow = (-1) * state["reserves_stables"] * params["max_outflow_rate"]* 2 / 3  # Smaller max_outflow_rate until target is first reached
        else:
            max_outflow = (-1) *state["reserves_stables"]* params["max_outflow_rate"] # Ensure that the reserve release is limited by max_outflow_rate

...
import pandas as pd
def plot_relative_kpi(kpi,kpidf,descriptors):
    reference_val = kpidf.loc[kpidf['face_value']==0][kpi].mean() # face_val == 0 means there's no bond
    comparative_vals = kpidf.loc[kpidf['face_value']!=0][kpi]-reference_val
    comparative_vals = pd.concat([comparative_vals,descriptors.loc[descriptors['face_value']!=0]],axis=1)
    for group_par in ['front_load_amount','face_value']:

def p_real_bid_capacity_cushion(_params, substep, state_history, state) -> dict:
    ...
    if (sum(state["bid_counter"]) >= _params["min_counter_reinstate"] or _params["with_reinstate_window"] == "No") and state["natural_price"] > state["lower_target_cushion"]:  # Refill capacity
        bid_capacity_cushion = state["bid_capacity_target_cushion"]
        ...
        bid_capacity_cushion = prev_day["bid_capacity_target_cushion"] + \
            state["net_flow"] - state["reserves_in"] + prev_day["liq_stables"] - \
            (state["amm_k"] + state["lower_target_cushion"]) ** (1/2)

    else:
        bid_capacity_cushion = prev_day["bid_capacity_cushion"]

    if bid_capacity_cushion < 0:
        bid_capacity_cushion = 0
    elif bid_capacity_cushion > state["bid_capacity_target_cushion"]:
        bid_capacity_cushion = state["bid_capacity_target_cushion"]
